...
public interface FortuneRecordMapper {
     ...
     * 插入命理记录
     */
    @Insert("INSERT INTO fortune_records (user_id, name, gender, birth_year, birth_month, birth_day, birth_hour, " +
     ...
     * 根据用户ID查询历史记录（分页）
     */
    @Select("SELECT * FROM fortune_records WHERE user_id = #{userId} AND deleted = 0 " +
     ...
     * 统计用户历史记录总数
     */
    @Select("SELECT COUNT(*) FROM fortune_records WHERE user_id = #{userId} AND deleted = 0")
    int countByUserId(@Param("userId") Long userId);
    
    /**
     * 根据ID查询记录
     */
    @Select("SELECT * FROM fortune_records WHERE id = #{id} AND deleted = 0")
    FortuneRecordPO selectById(@Param("id") Long id);
    
    /**
     * 删除记录（逻辑删除）
     */
    @Update("UPDATE fortune_records SET deleted = 1, updated_time = NOW() WHERE id = #{id}")
